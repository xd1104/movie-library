...
如果你已經在用[鑰匙圈](https://github.com/xd1104/keyring)，這個 App 也接上去了——
第一次打開會自己跳出解鎖畫面，**選自己 → 輸密碼 → 勾「記住這台裝置」**，金鑰就進來了，不用手貼。

鑰匙圈後台那邊要做的事（只做一次）：

1. 後台 →「登記新 App」，**代號填 `movie-library`**（一定要這串英文，不能用中文）、
   名字「好雷嗎」、emoji 🎬。
2. 在**你自己**那個人底下，把「好雷嗎」那一格貼上**這樣一整行**：

   ```json
   {"tmdb":"你的 TMDB 金鑰","omdb":"你的 OMDb 金鑰"}
   ```

   > 這個 App 要**兩把**金鑰，但鑰匙圈一格只放得下一串，所以打包成上面這種格式。
   > 沒有 OMDb 就寫 `{"tmdb":"你的 TMDB 金鑰","omdb":""}`。
   > **格式貼錯的話 App 的設定頁會直接告訴你哪裡不對、正確長怎樣**，不會靜靜壞掉。
3. 存檔（後台會自動發布）。手機打開 App → 解鎖一次。

> 沒勾「記住這台裝置」的話，金鑰只留在這個分頁，關掉就沒了——借別人手機用的時候用這個。

### B. 自己貼（沒有鑰匙圈，或鑰匙圈解不開時）

設定頁往下就是手貼的兩格，**這條路永遠留著**：鑰匙圈忘記密碼、被收回、或離線抓不到，
App 都還是能用。摘要：
...
```
  ...
  ...
  全部通過：共 859 個斷言，859 過 / 0 失敗，13 支測試檔
```
